...
- AI 对话链路：folder/document/resource-link 的既有 context menu（以及 document reader 的“更多”菜单）→ `KuzioCoworkConversationTarget` → `LibraryRootView.coworkTarget` → 右栏 `KuzioCoworkHarnessHost` → presentation-only `IntatisCoworkUI.IntatisCoworkContentView`。Kuzio host model 只把 selected context、既有 runtime lifecycle、公开 state/actions/thread source、模型选择与 `CodeItem` / `CoworkAgentThreadSnapshot` 输入映射到依赖 UI，再由 `KuzioCoworkRuntimeProfile` → `CodexRuntimeConfiguration(mode: .cowork)` → `CodexAppServerSession`。不得在 Kuzio 重新实现 transcript、composer、模型菜单、permission card、Inspector、Markdown/rich renderer、scroll coordinator 或 status rail；也不得添加 Kuzio 自制关闭叉号。启动前右栏只能显示占位且不得预启动 runtime；每次显式激活 request 使用稳定 `cowork_<request-uuid>` session identity。替换 target 或关闭主窗口必须 shutdown runtime，不能遗留 App Server process；不得恢复第二个 AI `WindowGroup`。
...
- 选中项上下文只把 `NodeID`、kind、title 与 virtual path 写入 owner-only session workspace `AGENTS.md`；不得写 raw external path、bookmark/locator 或 manifest fragment。`KuzioCodexLibraryTools` 通过 `com.vitemis.kuzio.library-tools.cowork.v2` toolset identity，把 `read_structure` / `read_content` / `mutate_structure` 对应的完整 8 个 provider tools 逐个直接投影为 `CodexRuntimeDynamicTools`。一个工具只能对应一个最小操作；不得新增 organize/apply-plan/batch 等复合工具，也不得在 adapter 中编排、重试、合并 conflict 或改写调用。virtual create-folder/rename/move/trash/restore 可由 Cowork 调用；permanent delete、document content mutation、relink、source refresh 与外部目标写入仍不得广告或执行。注册不等于逐工具 approval 已接线，真实 App Server mutation callback 与 approval 必须单独验收。
