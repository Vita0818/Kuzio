...
```text
CodexAppServerSession（mode: cowork）
        |
        | official dynamicTools callback
        v
...
当前 `IntatisCodexRuntime` 已把 capability-filtered `LibraryToolProvider.definitions` 经 `CodexRuntimeDynamicTools` 并入 Cowork 工具总和，并以最薄 adapter 转发 invocation/result；不得把 provider 类型、prompt、重试策略或模型状态写入 `LibraryStore`、manifest schema 或 virtual hierarchy，也不得在 failure 时改走另一 runtime/translator/backend。
