...
- browser root 的 32pt 大标题必须保持“资料库”；进入任意 virtual folder 后，大标题必须直接显示当前文件夹 title，breadcrumb 继续显示祖先路径。file resource-link 的两行标题必须使用中间截断并通过原生 hover help 提供完整标题；folder 保持默认尾部截断。不得为此增加第三行、状态文案、徽标或改变统一 tile 尺寸。
...
- 主 detail 必须保持原生 `HSplitView`：左侧学习库 min 480 / ideal 720，右侧 harness min 440 / ideal 620。未选择 target 时右栏只允许系统语义空态且不得预启动 runtime；激活后必须直接挂载presentation-only `IntatisCoworkUI.IntatisCoworkContentView`。Kuzio不得实现或复制transcript/message row、composer、provider/model menu、Send/Stop、permission dialog/card、Agents/Goal/Tasks inspector、Markdown/rich renderer、scroll coordinator、status rail或SharedUI control styling；本地view只允许最薄lifecycle、bindings、selected context、state/actions/thread source与runtime actions。用户明确不增加Kuzio关闭叉号，也不得用Intatis history/new action冒充关闭。
...
- 当前 Cowork conversation authorization 必须显式授予 `read_structure`、`read_content` 与 `mutate_structure`，并按 provider 固定顺序逐个广告完整 8 个工具：`library_get_state`、`library_list_children`、`library_read_content`、`library_create_folder`、`library_rename_node`、`library_move_node`、`library_trash_node`、`library_restore_node`。不得遗漏已确认的五个结构 mutation，也不得把它们合并成复合工具。注册不等于已完成逐工具 approval；approval 接线必须作为独立内核合同验收。
- 一个工具只能对应一个最小业务操作。不得为了减少工具数量或表面简洁新增 `organize`、`apply_plan`、`batch_mutation`、create-or-update 或其他把多个动作合并的工具；Agent loop 负责编排多次调用，adapter/control-plane 只执行单次已命名操作。工具层不得保存计划、自动重试、合并 revision conflict 或改写下一步。
...
- Cowork 8-tool specs 的 toolset identity 固定为 `com.vitemis.kuzio.library-tools.cowork.v2`，不得退回旧 read-only projection 使用的 provider ID。任何 specs 名称、schema、顺序或 capability surface 的不兼容变化都必须更换 toolset identity，让 Intatis 官方 resume check fail closed；不得本地伪造旧 thread migration。
...
- SwiftPM 与 XcodeGen 的 Intatis local-package path 和七个实际 product dependencies必须保持一致；`Kuzio.xcodeproj` 只能由 `project.yml` 重新生成对应 `XCLocalSwiftPackageReference`，不得手工接入另一份 package。
...
- 前端改动必须实际检查：sidebar 品牌/行高、全局字体、Finder folder icon、root/当前文件夹大标题、breadcrumb、每个 36pt control、可见圆面边缘坐标点击、Light/Dark、长短/单双行名称的等尺寸 folder grid、resource-link 中间截断与完整标题 hover help、reader、trash 与 sheet。resource-link 数据行为改动还必须检查单一 `+`、文件/文件夹多选、取消/确认、递归层级、空目录、single/batch relink、单 transaction、无重复 node、嵌套文件打开、manifest 无 absolute root path 与原文件未变。
- Cowork 改动至少覆盖：process-first Kuzio host identity、Runtime/UI API v1、session identity readback、Intatis-owned fixture config→redacted exact route、多个provider/model→secret-free UI options、选择后的route与原session/workspace/runtime root/8-tool toolset保持一致、完整8-tool names/schema/order、read与单次mutation result、selected NodeID instructions无raw path/credential且明确一工具一最小操作、43-test suite、单一`WindowGroup` + library/harness split、源码证明`IntatisCoworkContentView` direct mount且Harness view无`CoworkShell`/自制model menu/close action/`TextEditor`/ScrollView/confirmation dialog/Kuzio controls、空态不启动runtime、Xcode Debug/Release、双架构runtime static/signature/integrity、Light/Dark folder/file entry、Intatis composer/model menu ready、target replacement/window-close shutdown无残留process，以及installed production revision/真实链接/ready。没有实际发送的模型turn、切换模型后的远端turn、model-driven App Server tool callback、child delegation或逐工具approval round trip必须明确标为未验证。
