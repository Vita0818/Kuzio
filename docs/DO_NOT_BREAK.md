...
- 工具层必须保持 provider-neutral；未由用户明确确认前，不得加入模型 SDK、prompt、聊天 UI、MCP server、网络 endpoint、后台 agent 或 provider-specific schema。
- `LibraryToolProvider` 只代表 Kuzio 向未来完整工具总和贡献的工具集合，不得把它误写成模型唯一工具面，也不得在 Kuzio 内提前实现另一套 runtime/tool aggregator。未来聚合必须保留其他工具来源的独立 executor，并在重复 tool name 时 fail closed。
...
- 工具调用参数与稳定错误码的事实源是 `docs/TOOL_CALLING.md`；未来 adapter 必须做最薄类型转换，不得改写工具语义。
